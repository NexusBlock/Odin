...
balanceInit=$(docker exec bor0 bash -c "bor attach /root/.bor/data/kernel.ipc -exec 'Math.round(web3.fromWei(eth.getBalance(eth.accounts[0])))'")
...
do
    ...
    balance=$(docker exec bor0 bash -c "bor attach /root/.bor/data/kernel.ipc -exec 'Math.round(web3.fromWei(eth.getBalance(eth.accounts[0])))'")
